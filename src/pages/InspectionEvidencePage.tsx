...
import type { InspectionEvidence, InspectionResult } from '../types'
...
export function InspectionEvidencePage() {
  ...
  return <div><div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-mono text-xs text-muted">{result.inspectionId}</p><h1 className="mt-1 text-2xl font-semibold tracking-tight text-ink">Evidence review</h1><p className="mt-1 text-sm text-muted">Supporting evidence for {result.productName}</p></div><div className="flex flex-wrap gap-2"><Button variant="secondary" size="sm" onClick={() => navigate(`/inspections/${id}/result`)}>Back to result</Button><Button size="sm" onClick={() => navigate(`/inspections/${id}/report`)}>View report <span aria-hidden="true">→</span></Button></div></div>{!evidence.length ? <Card className="py-14 text-center"><p className="font-semibold text-ink">No evidence returned</p><p className="mt-1 text-sm text-muted">The service did not return supporting evidence for this inspection.</p></Card> : <div className="grid gap-6 xl:grid-cols-[16rem_minmax(0,1fr)]"><Card className="h-fit p-4" title="Findings" description="Select an item to inspect its supporting image."><div className="mt-4 space-y-2">{evidence.map((item, index) => <button type="button" key={item.id} onClick={() => setSelectedId(item.id)} className={`w-full rounded-md border p-3 text-left transition-[background-color,border-color,transform] duration-150 ${selectedId === item.id ? 'border-brand bg-brand-light shadow-sm' : 'border-border hover:-translate-y-px hover:bg-bg'}`} aria-current={selectedId === item.id ? 'true' : undefined}><span className="text-xs font-medium text-muted">Evidence {index + 1}</span><span className="mt-1 block text-sm font-medium text-ink">{item.title}</span><span className="mt-1 block text-xs text-muted">{item.boundingBox ? 'Highlighted region returned' : 'Full image returned'}</span></button>)}</div></Card>{selected ? <EvidenceViewer evidence={selected} /> : <Card className="py-14 text-center"><p className="font-semibold text-ink">Select a finding</p><p className="mt-1 text-sm text-muted">Choose a finding to inspect its returned evidence.</p></Card>}</div>}</div>
}
